...
export const unicode = {
  ...
  charWidth(str: string | number, i?: number): number {
    ...
    // tab
    if (point === 0x09) {
      // Note: This reference to blessed is problematic in TypeScript
      // and would need to be refactored later
      if (!global.blessed) {
        try {
          global.blessed = require('../..');
        } catch (e) {
          return 8; // Default tab width if blessed is not available
        }
      }
      return global.blessed?.screen?.global
        ? global.blessed.screen.global.tabc.length
        : 8;
    }
    ...
    // Wide and Full-width characters
  ...
  strWidth(str: string): number {
    ...
    // Use GraphemeSplitter for improved handling of grapheme clusters
    const splitter = new GraphemeSplitter();
    const graphemes = splitter.splitGraphemes(str);

    for (const grapheme of graphemes) {
      // This is a simplification - a proper implementation would
      // need to handle each grapheme's width based on all its codepoints
      if (grapheme.length === 1) {
        width += unicode.charWidth(grapheme, 0);
      } else {
        // For complex graphemes, we need to consider which codepoint determines width
        // Typically the base character, not combining marks
        width += unicode.charWidth(grapheme.codePointAt(0) || 0);
      }
    }
  ...
  /**
   * Check if a character is a surrogate pair
   *
   * @param str String to check
  ...
  // Table of combining character ranges
  combiningTable: [
  ...
  // Convert combining table to an object for fast lookup
  get combining() {
    const table = {} as Record<number, boolean>;
    for (const row of this.combiningTable) {
   ...
   *
   * @param str String or code point to check
   * @param i Position in the string (used when str is a string)
   * @returns True if the character is a combining character
   */
  isCombining(str: string | number, i?: number): boolean {
    const point = typeof str !== 'number'
      ? this.codePointAt(str, i || 0)
      : str;
    return this.combining[point] === true;
  },

  /**
   * Gets the code point at a given position
   *
   * @param str String to check
   * @param position Position in the string
   * @returns The code point at the position
   */
  codePointAt(str: string, position: number): number {
    if (str == null) {
      throw TypeError('String cannot be null or undefined');
    }

    // Try native implementation first
    const nativeResult = str.codePointAt(position);
    if (nativeResult !== undefined) {
      return nativeResult;
    }

    // Fallback implementation for environments without codePointAt
    const string = String(str);
    const size = string.length;

    // `ToInteger`
    let index = position ? Number(position) : 0;
    if (Number.isNaN(index)) { // better `isNaN`
      index = 0;
    }
   ...
   *
   * @param codePoints Sequence of code points
   * @returns String created from the code points
   */
  ...
  chars: {
    ...
    // This will be set by the code below
    combining: null as RegExp | null
  } as IUnicodeChars,
};
...
// Create regex for combining characters
const combiningRegex = new RegExp(
  unicode.combiningTable.reduce((out, row) => {
    let low, high, range;
    if (row[0] > 0x00ffff) {
      const lowChar = unicode.fromCodePoint(row[0]);
      low = [
        hexify(lowChar.charCodeAt(0)),
        hexify(lowChar.charCodeAt(1))
      ];
      const highChar = unicode.fromCodePoint(row[1]);
      high = [
        hexify(highChar.charCodeAt(0)),
        hexify(highChar.charCodeAt(1))
      ];
...
// Add to chars object
unicode.chars.combining = combiningRegex;
