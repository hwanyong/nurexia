...
// Type definitions
export interface ICombiningTable extends Array<[number, number]> {}
...
export function strWidth(str: string): number {
  ...
  for (const grapheme of graphemes) {
    ...
    if (grapheme.length === 1) {
      ...
      width += charWidth(grapheme.codePointAt(0) || 0);
    }
  }
...
export const combiningTable: ICombiningTable = [
...
export const combining: Record<number, boolean> = combiningTable.reduce((out, row) => {
...
export const chars: IUnicodeChars = {
...
export const combiningRegex = new RegExp(
  combiningTable.reduce((out, row) => {
    let low, high, range;
    if (row[0] > 0x00ffff) {
      const lowChar = fromCodePoint(row[0]);
      low = [
        hexify(lowChar.charCodeAt(0)),
        hexify(lowChar.charCodeAt(1))
      ];
      const highChar = fromCodePoint(row[1]);
      high = [
...
// Add to chars object
chars.combining = combiningRegex;
